...
void BOD_Open(uint8_t u8Mode, uint8_t u8BODLevel)
{
    ...
    BOD->BODSEL = (BOD->BODSEL & ~BOD_BODSEL_BODVL_Msk) | u8BODLevel;
    BOD->BODCTL = (BOD->BODCTL & ~BOD_BODCTL_BODEN_Msk) | u8Mode;
...
{
	BOD->BODDTMR = (BOD->BODDTMR & ~(BOD_BODDTMR_DURTON_Msk | BOD_BODDTMR_DURTOFF_Msk)) | (((u8OnDUR<<BOD_BODDTMR_DURTON_Pos)&BOD_BODDTMR_DURTON_Msk) | (u16OffDUR&BOD_BODDTMR_DURTOFF_Msk));
}
